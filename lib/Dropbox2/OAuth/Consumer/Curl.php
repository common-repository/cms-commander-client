...
class CMSC_Dropbox_OAuth_Consumer_Curl extends CMSC_Dropbox_OAuth_Consumer_ConsumerAbstract {
 ...
 public function fetch($method, $url, $call, array $additional = array(), $isChunkDownload = array())
	{
		...
		if ($response === false) {
			throw new CMSC_Dropbox_CurlException($error);
		} else {
			...
			if (!empty($response['body']->error) || $code >= 400) {
				...
				switch ($code) {
					case 304:
						throw new CMSC_Dropbox_NotModifiedException($message, 304);
					case 400:
						throw new CMSC_Dropbox_BadRequestException($message, 400);
					case 404:
						throw new CMSC_Dropbox_NotFoundException($message, 404);
					case 406:
						throw new CMSC_Dropbox_NotAcceptableException($message, 406);
					case 415:
						throw new CMSC_Dropbox_UnsupportedMediaTypeException($message, 415);
					case 401:
						//401 means oauth token is expired continue to manually handle the exception depending on the situation
						throw new CMSC_Dropbox_TokenExpired($message, 401);
						// continue;
					case 409:
						//409 in API V2 every error will return with a 409 to find out what the error is the error description should be checked.
						throw new CMSC_Dropbox_Exception($message, $code);
					default:
						throw new CMSC_Dropbox_Exception($message, $code);
				}
			}
